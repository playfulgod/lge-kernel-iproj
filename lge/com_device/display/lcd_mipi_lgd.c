...
static int mipi_lgd_lcd_on(struct platform_device *pdev)
{
	...
	if(!is_lcd_power_on) {
		mipi_lgd_lcd_reset();	
		// device initialize	
		mipi_dsi_cmds_tx(&lgd_tx_buf, lgd_power_on_set ,ARRAY_SIZE(lgd_power_on_set));
		mipi_dsi_cmds_tx(&lgd_tx_buf, main_sleep_out , ARRAY_SIZE(main_sleep_out));
		mipi_dsi_cmds_tx(&lgd_tx_buf, main_display_on , ARRAY_SIZE(main_display_on));
		is_lcd_power_on = 1;
	}
	else{
		mipi_lgd_lcd_reset();	
		mipi_dsi_cmds_tx(&lgd_tx_buf, lgd_power_on_set ,ARRAY_SIZE(lgd_power_on_set));
		mipi_dsi_cmds_tx(&lgd_tx_buf, main_lcd_sleep_out , ARRAY_SIZE(main_lcd_sleep_out));
...
static int mipi_lgd_lcd_off(struct platform_device *pdev)
{
	...
	mipi_dsi_cmds_tx(&lgd_tx_buf, main_lcd_sleep_in, ARRAY_SIZE(main_lcd_sleep_in));
